...
WRITE_CODE_GUIDANCE = (
    ...
    "the inflation gap. After computing YoY and inner-joining dates, keep only "
    "the last N*12 monthly YoY observations ending at the latest common month "
    "(60 months for a last-5-years question). Compute the average gap, chart, "
    "and tables on that filtered window; do not average the extra lookback months "
    "that exist only so YoY can be formed. Name the average gap after the actual "
    "overlapping YoY window; calling it a five-year average is OK when that "
    "filtered window has at least 60 months. YoY MUST be calendar-grounded: for month M use the "
...
SIMPLE_RETRY_WRITE_CODE_GUIDANCE = (
    ...
    "inner-join dates, keep the last 60 YoY months (or the requested window) "
    "ending at the latest common date, numeric metric values, and one chart "
    "matching schema X "
