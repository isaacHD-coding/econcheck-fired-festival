...
- OpenAI mode uses local templates for those same known classes (no OpenAI `write_code` / `design_chart` / `draft` call). A CPI vs PCE question is a two-series **calendar** year-over-year gap (same month minus 12 months via date keys; no positional 12-lag, no LOCF past the last raw month). Raw FRED history for N years of YoY is at least N+2 years (seven years for “last 5 years” inflation rates) so the overlap is not truncated to ~46 months. After inner-join, analysis is **trimmed to the requested window** (last 60 monthly YoY observations ending at the latest common month for a 5-year ask); extra lookback months are only used to form YoY. The average-gap metric is named after that filtered window; calling it five years is OK at ≥60 months. The draft also notes why CPI and PCE can differ (basket, weights, formula). Canonical CPI fallback remains limited to the exact demo CPI question with CPI-only data. If OpenAI still writes analysis code (unknown question classes), the prompt requires the exact AnalysisArtifact metrics/charts schema (numeric `metrics[].value`, `charts[].data` rows — not nested `charts[].series`). A size checkpoint rejects scripts over 300 nonempty lines or 8 helpers before the sandbox runs them. If generated analysis fails MathSanity, ChartPromise, **or checker review** on a CPI vs PCE two-series fetch, the harness swaps in the compact calendar-YoY template instead of another OpenAI novel (at most two checker `code_generation` retries). Checker `data_discovery` retries that would refetch the same lookback escalate instead of looping. Timeouts are unchanged (write_code 120s, whole-run 180s).
...
- Mixed-frequency relationship analysis aligns series by carrying higher-frequency values forward onto lower-frequency dates and reports contemporaneous growth-rate correlation, not a causal or full lead-lag model. CPI vs PCE uses **calendar** year-over-year percent changes (same month last year via date keys, inner-joined; missing months are dropped, not positionally lagged). The harness fetches at least two extra years of raw index levels so five years of YoY can actually be formed, then trims the analysis to the last 60 overlapping YoY months for a 5-year question. Index bases differ. Observability shows `chart_brief.json` next to analysis artifacts.
